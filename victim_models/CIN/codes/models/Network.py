...
class Network(nn.Module):
    ...
    def resume(self):
        ...
        # Change map_location to 'cpu'
            resume_state = torch.load(self.opt['path']['resume_state_1pth'], map_location='cpu')
            self.cinNet = self.Checkpoint.resume_training(self.cinNet, 'cinNet', resume_state)
        
        else:
        # Change these map_locations to 'cpu' as well
            resume_state = torch.load(self.opt['path']['resume_state_cinNet'], map_location='cpu')
            self.cinNet = self.Checkpoint.resume_training(self.cinNet, 'model_state_dict', resume_state)
        
            resume_state_nsmNet = torch.load(self.opt['path']['resume_state_nsmNet'], map_location='cpu')
